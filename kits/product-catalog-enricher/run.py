#!/usr/bin/env python3
"""
Product Catalog Enrichment Agent — runnable demo (Agent Blueprint from AgentKits)
...
Original blueprint: https://agent-kits.com/kit/product-catalog-enricher
...
SYSTEM_PROMPT = """You are a Product Catalog Enrichment Agent. You enrich product catalog entries (descriptions, attributes, categories, SEO fields) using the PROVIDED source data. You are judged on rich, consistent, accurate entries and on never fabricating a spec, attribute, or claim.
...
SAMPLE_INPUT = """Source: 'Stainless steel frying pan, 28cm, induction-compatible, dishwasher safe.'
"""

# Tools this blueprint exposes (name -> purpose). All are MOCK stubs here.
TOOL_PURPOSES = {
    "get_product": "Retrieve the product's source data.",
    "extract_attributes": "Pull structured attributes from the source.",
    "enrich_description": "Write a source-grounded product description.",
    "categorize": "Assign the product to the taxonomy consistently.",
    "seo_fields": "Generate SEO title and meta fields from grounded content.",
    "flag_uncertain": "Flag missing or uncertain attributes for review.",
    "fact_guard": "Block fabricated specs and claims; flag those needing verification.",
    "validate_taxonomy": "Keep categorization and attribute naming consistent.",
}
...
def main():
    print("=" * 70)
    print("Product Catalog Enrichment Agent" + " — runnable demo (MOCK tools, real reasoning)")
    print("=" * 70)
    print("INPUT:\n" + SAMPLE_INPUT)
    print("-" * 70)
