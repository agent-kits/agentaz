#!/usr/bin/env python3
"""
Action Item Tracking Agent — runnable demo (Agent Blueprint from AgentKits)
...
Original blueprint: https://agent-kits.com/kit/action-item-tracker
...
SYSTEM_PROMPT = """You are an Action Item Tracking Agent. You extract action items from a meeting (notes or transcript) and track them. You are judged on faithful, useful action items and on never inventing tasks, owners, or due dates, or committing people to work they didn't agree to.
...
SAMPLE_INPUT = """Transcript: 'John: I'll send the updated deck to the client by Friday.'
"""

# Tools this blueprint exposes (name -> purpose). All are MOCK stubs here.
TOOL_PURPOSES = {
    "get_transcript": "Retrieve the meeting notes or transcript.",
    "extract_action_items": "Identify explicitly agreed action items from the meeting.",
    "identify_owner": "Assign an owner only where the meeting clearly established one.",
    "identify_due_date": "Capture a due date only where one was stated.",
    "flag_ambiguous": "Flag unclear ownership or missing dates for confirmation.",
    "track_status": "Track each action item's status over time.",
    "draft_followup": "Draft reminders/follow-ups for review rather than auto-sending.",
    "link_to_source": "Tie each item to where it came up in the meeting.",
}
...
def main():
    print("=" * 70)
    print("Action Item Tracking Agent" + " — runnable demo (MOCK tools, real reasoning)")
    print("=" * 70)
    print("INPUT:\n" + SAMPLE_INPUT)
    print("-" * 70)
