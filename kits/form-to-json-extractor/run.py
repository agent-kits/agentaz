#!/usr/bin/env python3
"""
Form-to-JSON Extraction Agent — runnable demo (Agent Blueprint from AgentKits)
...
Original blueprint: https://agent-kits.com/kit/form-to-json-extractor
...
SYSTEM_PROMPT = """You are a Form-to-JSON Extraction Agent. You read a filled-in form/document and output schema-valid JSON of the fields, with per-field confidence. You are judged on accurate, faithful extraction and on NEVER guessing, inventing, or inferring a field value that isn't actually on the document.
...
SAMPLE_INPUT = """A typed application form with Name: Jane Smith, DOB: 1990-04-12, State: active — all legible.
"""

# Tools this blueprint exposes (name -> purpose). All are MOCK stubs here.
TOOL_PURPOSES = {
    "get_document": "Retrieve the form/document and the target schema.",
    "detect_fields": "Locate each schema field's value on the document.",
    "extract_to_schema": "Extract values as written and map them to schema fields.",
    "confidence_score": "Attach a confidence to each extracted value.",
    "validate_schema": "Validate the output against types, required fields, and enums.",
    "flag_low_confidence": "Flag low-confidence and ambiguous fields for review.",
    "flag_missing": "Mark blank or absent fields as missing rather than guessing.",
    "ocr_quality_check": "Assess OCR/legibility and flag illegible fields.",
}
...
def main():
    print("=" * 70)
    print("Form-to-JSON Extraction Agent" + " — runnable demo (MOCK tools, real reasoning)")
    print("=" * 70)
    print("INPUT:\n" + SAMPLE_INPUT)
    print("-" * 70)
